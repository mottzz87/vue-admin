...
<template>
	<el-form ref="formRef" :model="form" v-bind="getFormAttrs">
		<el-row :gutter="10">
			...
			<el-col v-if="actions?.save" class="save-btn">
			...
			<div
				v-else-if="actions?.search && formItems?.length"
				class="search-btn"
				:style="{
...
export default {
	...
	setup(props, { attrs, emit }) {
		let { fields, formData, actions } = props?.config;
		...
		const delCusPropers = (obj: Object) => {
			...
			const cusPropersArr = ['show', 'width', 'prop', 'label', 'is'];
		...
		const getComsAttrs = (col: any) => {
			...
			const isSelect = /^(Select|DatePicker|CheckboxGroup|Checkbox)/.test(is);
		...
		const formItems = computed(() => {
			...
			// 针对筛选条件自定义输入框宽度
			if (actions?.search) {
				_fields = fields.map((v: { is: string | number; type: String }) => ({
					...(widthConfig[v.is] && v?.type.includes('range') ? widthConfig[v.is] : { width: 258 }),
					...v,
				}));
			}
		...
		const reset = () => {
			...
			if (Object.keys(query).length && actions.search?.isResetRouteQuery) {
				return reload(path);
			}
			// reset表单
